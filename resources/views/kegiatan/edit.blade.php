            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group mb-2">
                        ...
                        <select name="id_paket" class="form-control" id="kegiatan-dd">
                            <option value=""></option>
                            @foreach ($paket as $row)
                                <option @if ($kegiatan->id_paket == $row->id_paket) selected @endif value="{{ $row->id_paket }}">{{ $row->nama_paket }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group mb-2">
                        <label for="">Kupon</label>
                        <select name="id_kupon" class = "form-control" id="sub-dd">
                            <option value=""></option>
                            @foreach ($kupon as $sub)
                                <option @if ($kegiatan->id_kupon == $sub->id_kupon) selected @endif value="{{ $sub->id_kupon }}">{{ $sub->kode_kupon }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group mb-2">
                    ...
                    <div class="form-group mb-2">
                        ...
                        <textarea name="deskripsi_kegiatan" class="form-control" id="editor" cols="30" rows="10">{{ $kegiatan->deskripsi_kegiatan }}</textarea>
                    </div>
                    <div class="form-group mb-2">
